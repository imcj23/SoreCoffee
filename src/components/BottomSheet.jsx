...
const BottomSheet = ({ menu, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <div
        ref={sheetRef}
        className={`fixed z-50 bg-white shadow-2xl transform transition-all duration-1000 max-h-[90vh] overflow-y-auto ${
          isClosing
            ? "translate-y-full opacity-100"
            : "translate-y-0 opacity-100"
        }
        w-full
        md:max-w-md md:mx-auto md:my-4 md:rounded-2xl
        bottom-0 left-0 right-0 rounded-t-3xl
        md:bottom-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2
        md:transform-none ${isClosing ? 'md:scale-95 md:opacity-0' : 'md:scale-100 md:opacity-100'}
        `}
        style={{
      ...
      >
        <div className="sticky top-0 bg-white pt-4 pb-2 rounded-t-3xl md:rounded-t-2xl z-10">
        ...
        <div className="px-4 md:px-6 pb-6 md:pb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
            <div className="flex-1">
              <h3 className="text-xl md:text-2xl font-bold text-gray-900">{menu.name}</h3>
          ...
          <p className="text-gray-600 text-sm md:text-base mb-6">{menu.description}</p>
            ...
            <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4">
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="w-10 h-10 rounded-full bg-amber-900 text-white flex items-center justify-center hover:bg-amber-800 transition-colors"
                >
                  <FaPlus size={16} />
                </button>
              </div>
          ...
          <div className="sticky md:static bottom-0 left-0 right-0 bg-white md:bg-transparent pt-4 md:pt-0 border-t md:border-none">
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={handleClose}
                className="px-6 py-3 border-2 border-amber-900 text-amber-900 font-bold rounded-xl hover:bg-amber-50 transition-colors duration-300 flex-1"
              >
                Lihat Menu Lain
              </button>
              <button
